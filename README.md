...
- `image` — загрузка, сохранение и хранение grayscale-изображения
- `filter` — последовательные фильтры, ядра свёртки и median filter
- `parallel` — параллельные стратегии разбиения изображения
- `app` — основная логика командной строки
- `Main` в корне — совместимая точка входа для старого запуска
...
### Результаты исследования второго задания

Для второго задания проведены замеры на изображениях из `D:\temp`:
...
Проверялись фильтры `gaussian3`, `gaussian5`, `blur5`, `sharpen3`, `motion9`, `median3`, `median5`.
Для каждого фильтра проверены все стратегии разбиения изображения: `pixels`, `rows`, `columns`, `grid`.
Количество потоков: `1`, `2`, `4`, `8`.

Полная таблица результатов сохранена в файле:

```text
research/task2/parallel_results.csv
```

Подробный отчёт находится здесь:

```text
research/task2/README.md
```

#### График времени выполнения

![Среднее время параллельной обработки](research/task2/parallel_time_by_threads.png)

#### График ускорения

![Ускорение параллельной обработки](research/task2/parallel_speedup_by_threads.png)

#### Сравнение стратегий разбиения

![Сравнение стратегий](research/task2/parallel_strategy_comparison.png)

#### Пропускная способность

![Пропускная способность](research/task2/parallel_throughput_by_image.png)

#### Время всех фильтров и стратегий на разных разрешениях

На этих графиках показаны все проверенные фильтры на разных разрешениях.
Для каждого фильтра сравнивается последовательная версия и все четыре стратегии разбиения при `8` потоках.

![Время всех фильтров 256x256](research/task2/parallel_all_filters_time_256x256.png)

![Время всех фильтров 512x512](research/task2/parallel_all_filters_time_512x512.png)

![Время всех фильтров 1024x1024](research/task2/parallel_all_filters_time_1024x1024.png)

![Время всех фильтров 2048x2048](research/task2/parallel_all_filters_time_2048x2048.png)

#### Ускорение всех фильтров и стратегий на разных разрешениях

На этих графиках показано ускорение всех параллельных стратегий для каждого фильтра на разных разрешениях.
Так видно не только лучший результат, но и поведение всех алгоритмов разбиения.

![Ускорение всех фильтров 256x256](research/task2/parallel_all_filters_speedup_256x256.png)

![Ускорение всех фильтров 512x512](research/task2/parallel_all_filters_speedup_512x512.png)

![Ускорение всех фильтров 1024x1024](research/task2/parallel_all_filters_speedup_1024x1024.png)

![Ускорение всех фильтров 2048x2048](research/task2/parallel_all_filters_speedup_2048x2048.png)

#### Время при разном количестве потоков

На этих графиках для каждого фильтра сравниваются `1`, `2`, `4` и `8` потоков.
Для каждого количества потоков выбран лучший результат среди стратегий `pixels`, `rows`, `columns`, `grid`.

![Время при разном количестве потоков 256x256](research/task2/parallel_threads_time_256x256.png)

![Время при разном количестве потоков 512x512](research/task2/parallel_threads_time_512x512.png)

![Время при разном количестве потоков 1024x1024](research/task2/parallel_threads_time_1024x1024.png)

![Время при разном количестве потоков 2048x2048](research/task2/parallel_threads_time_2048x2048.png)

#### Ускорение при разном количестве потоков

На этих графиках видно, как меняется ускорение при увеличении числа потоков.

![Ускорение при разном количестве потоков 256x256](research/task2/parallel_threads_speedup_256x256.png)

![Ускорение при разном количестве потоков 512x512](research/task2/parallel_threads_speedup_512x512.png)

![Ускорение при разном количестве потоков 1024x1024](research/task2/parallel_threads_speedup_1024x1024.png)

![Ускорение при разном количестве потоков 2048x2048](research/task2/parallel_threads_speedup_2048x2048.png)
...
#### Краткий вывод по второму заданию

На маленьком изображении `256x256` параллельная обработка не всегда быстрее последовательной, потому что накладные расходы на потоки становятся заметными.
На больших изображениях параллельная версия даёт стабильное ускорение.

Для `gaussian5` на `2048x2048` последовательная версия выполнилась за `347.144 мс`, а лучший параллельный вариант (`grid`, `8` потоков) — за `96.145 мс`.
Ускорение составило `3.611` раза.

Лучше всего себя показали стратегии `rows`, `columns` и `grid`.
Стратегия `pixels` работает хуже, потому что каждый пиксель берётся через общий атомарный счётчик, и это добавляет overhead.
